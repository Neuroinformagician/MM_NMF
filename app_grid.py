...
def transform_to_nmf_features(input_data, models):
    ...
    # 入力データを正しい順序で配列化
    input_array = np.array([[input_data.get(col, 0) for col in df_columns]])
    ...
    # NMF変換
    W = models['nmf'].transform(input_scaled)
...
def display_diamond_chart(W_df, group_stats):
    """ダイヤモンドチャート表示"""
...
def display_results(predictions, probabilities, W_df, group_stats, mgadl_total, mgc_total, mgqol_total):
    """結果表示"""
    st.markdown("---")
    ...
    # ダイヤモンドチャート
    st.markdown("### 💎 群間比較")
    fig = display_diamond_chart(W_df, group_stats)
    ...
    with col2:
        if st.button("🚀 予測を実行", type="primary", use_container_width=True):
            ...
            if total_score == 0:
                ...
                with st.spinner("予測中..."):
                    # NMF特徴量に変換
                    W_df = transform_to_nmf_features(st.session_state.scores, models)
